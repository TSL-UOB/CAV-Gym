...
actors = [
    ...
    PelicanCrossing(
        ...
        constants=PelicanCrossingConstants(
            road=road_layout.major_road,
            width=road_layout.major_road.constants.lane_width * 1.5,
            x_position=road_layout.major_road.constants.length / 2.0
        )
    ),
    Pedestrian(
        init_state=DynamicActorState(
            position=utilities.Point(env_constants.viewer_width / 2.0, -(env_constants.viewer_height / 2.0) + (road_layout.major_road.constants.lane_width * 0.75)),
            velocity=0.0,
