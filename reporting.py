...
class EpisodeResults(LogMessage):
    ...
    def console_message(self):
        ...
        return f"episode {self.index} {episode_status} after {self.time.timesteps} timestep(s) in {utilities.pretty_float(self.time.runtime(), decimal_places=0)} ms ({utilities.pretty_float(self.time.simulation_speed())}:1 real-time), {test_status}"
...
class Interval:
    ...
    def pretty(self, **kwargs):
        return f"({utilities.pretty_float(self.min, **kwargs)}, {utilities.pretty_float(self.max, **kwargs)})"
...
class SuccessfulTestResults:
    ...
    def __post_init__(self):
        assert self.count > 0 or (not self.total_timesteps and not self.total_runtime and not self.total_score)
...
class RunResults(LogMessage):
    ...
    def console_message(self):
        ...
        return f"run completed after {self.episodes} episode(s) and {self.time.timesteps} timestep(s) in {utilities.pretty_float(self.time.runtime(), decimal_places=0)} ms ({utilities.pretty_float(self.time.simulation_speed())}:1 real-time), {test_status}"
